...
#Make entrop and entalpy empty lists
Entropy=[]
Entalpy=[]
...
for i in range(msrmnt):
            ...
            if i==0:                           # calculate entalpy
                entalpy=(Ene)
            else:
                entalpy=(Ene-E[i-1])
                
            if i==0:                           # calculate entropy
                entropy=(Ene)/temp
            else:
                entropy=(Ene-E[-1])/temp+Entropy[-1]
...
#Plot evolution of Energy,Magnetization and Entropy during the simulation
plt.subplot(2, 1, 1)
plt.plot(step, E, 'r+-')
plt.ylabel('Energy')
...
#Entalpy plot
Entalpy.pop(0)
plt.plot(step, Entalpy, 'g+-')
plt.ylabel('ΔH')
plt.xlabel('MC step')
plt.title('Entalpy (ΔH)')

plt.show()

#Entropy plot
Entropy.pop(0)
plt.plot(step, Entropy, 'g+-')
plt.ylabel('ΔS')
plt.xlabel('MC step')
plt.title('Entropy (ΔS)')
